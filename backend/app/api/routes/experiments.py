...
async def join_experiment_by_id(
    ...
    for whitelist_item in all_experiment_id_items:
        collaborators.append(await users_repo.get_user_by_id(id=whitelist_item.user_id))
        if whitelist_item.peer_public_key == experiment_join_input.peer_public_key:
            raise HTTPException(
    ...
    _ = await whitelist_repo.update_item_by_ids(
        experiment_id=id,
        user_id=user_id[0],
        item_update=WhitelistItemUpdate(peer_public_key=experiment_join_input.peer_public_key),
    )
    ...
    exp_pass = ExperimentJoinOutput(**experiment_full.dict(), hivemind_access=hivemind_access)
    return exp_pass
